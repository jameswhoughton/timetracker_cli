...
func TestReturnMessageIfNoSessions(t *testing.T) {
	...
	cmd := NewListCmd(tracker, ListConfig{})
...
func TestIncludeRoundedTotalIfRoundByGreaterThan0(t *testing.T) {
	tracker := internal.NewTracker(internal.TrackerConfig{})
	...
	cmd := NewListCmd(tracker, ListConfig{RoundBy: 15})
	b := bytes.NewBufferString("")
	cmd.SetOut(b)
	cmd.Execute()
	...
	expectedTable := table.New("ID", "Description", "Start", "End", "Length", "Rounded")
	expectedTable.AddRow("1", "test", "01/01/70 00:00:00", "01/01/70 00:01:40", "1m 40s", "1m 45s")
	expectedTable.AddRow("2", "test 2", "01/01/70 00:00:30", "01/01/70 00:00:40", "10s", "15s")
	expectedTable.AddRow("3", "test 3", "01/01/70 00:01:00", "01/01/70 00:01:05", "5s", "15s")
